...
public class TSPSeedDNAv1 {
    ...
    static int calculateCost(int[][] graph, int[] route) {
        ...
        int n = route.length;
        for (int i = 0; i < n; i++) {
            cost += graph[route[i]][route[(i + 1) % n]];
        }
        return cost;
    }
    ...
    static int calculateCost(int[][] graph, List<Integer> route) {
        ...
        int n = route.size();
        for (int i = 0; i < n; i++) {
            cost += graph[route.get(i)][route.get((i + 1) % n)];
        }
        return cost;
    }
        ...
        while (improved) {
            ...
            for (int i = 1; i < n - 1; i++) {
                for (int j = i + 1; j < n; j++) {
                    if ((j + 1) % n == i - 1)
                        continue;
                    int currentEdge1 = graph[route[i - 1]][route[i]];
                    int currentEdge2 = graph[route[j]][route[(j + 1) % n]];
                    int newEdge1 = graph[route[i - 1]][route[j]];
                    int newEdge2 = graph[route[i]][route[(j + 1) % n]];
    ...
    // ==========================================
    static Result tspSeedDNA(int[][] graph, Random rand, int testCaseId, List<String> inputCsvLines,
        ...
        while (newGroupsFormed && iter < maxIterations) {
            ...
            List<int[]> routes = IntStream.range(0, routeCount)
                    .parallel()
                    .mapToObj(r -> {
            ...
            if (iter == 1) {
                for (int r = 0; r < sortedRoutes.size(); r++) {
                    ...
                    inputCsvLines.add(String.format("SEED_ROUTE,%d,%d,%d,%s", testCaseId, r + 1, cost, sb.toString()));
                }
            }

            // Build pre-computed hash lookup for containment checks
            Map<String, List<Integer>> seedToRouteIndices = new HashMap<>();
            for (int r = 0; r < sortedRoutes.size(); r++) {
                int[] route = sortedRoutes.get(r);
                for (int i = 0; i < n; i++) {
                    int u = route[i];
                    ...
                    // Triplet key
                    String tKey = TripletSeed.getTripletKey(u, v, w);
            ...
            Map<String, Integer> tripletFreq = new HashMap<>();
            Map<String, TripletSeed> tripletObjects = new HashMap<>();
            ...
            for (int[] route : sortedRoutes) {
                for (int i = 0; i < n; i++) {
                    ...
                    int v = route[(i + 1) % n];
                    int w = route[(i + 2) % n];
                    TripletSeed ts = new TripletSeed(u, v, w);
                    String key = ts.key;
                    tripletFreq.put(key, tripletFreq.getOrDefault(key, 0) + 1);
                    tripletObjects.put(key, ts);
                }
            }

            // Identify repeating triplets in current iteration (frequency >= 2)
            List<TripletSeed> currentIterationSeeds = new ArrayList<>();
            for (Map.Entry<String, Integer> entry : tripletFreq.entrySet()) {
                if (entry.getValue() >= 2) {
                    currentIterationSeeds.add(tripletObjects.get(entry.getKey()));
                }
            }

            // 6. Compare seeds from Iteration N and N+1, add weight (+0.9) for repeating
            // seeds
            for (TripletSeed ts : currentIterationSeeds) {
                if (globalSeeds.containsKey(ts.key)) {
                    ...
                    List<Integer> rIds = seedToRouteIndices.get(ts.key);
            ...
            for (Map.Entry<String, Integer> entry : pairAcrossSeedsCount.entrySet()) {
                if (entry.getValue() >= 2) {
                    ...
                    if (!globalSeeds.containsKey(pKey)) {
                        ...
                        List<Integer> rIds = seedToRouteIndices.get(pKey);
            ...
            // 8. Reduce weight (-0.3) for Seeds if they increase the cost in any 3
            // iterations
            int cMin = sortedCosts.get(0);
            for (Seed s : globalSeeds.values()) {
                List<Integer> rIds = seedToRouteIndices.get(s.key);
        ...
        // Finalize route by stitching sorted seeds
        return finalizeRoute(graph, globalSeeds);
    }
