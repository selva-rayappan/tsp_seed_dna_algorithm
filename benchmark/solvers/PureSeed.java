...
public class PureSeed implements TSPSolver {

    private int threads = Runtime.getRuntime().availableProcessors();
    private int routeCountMultiplier = 3;

    public PureSeed() {
    }

    public PureSeed(int threads) {
    ...
    public int[] solve(double[][] cities) {
        ...
        Result res = tspSeedDNA(cities, graph, new Random(42), routeCountMultiplier);
    ...
    Result tspSeedDNA(double[][] cities, int[][] graph, Random rand, int routeCountMultiplier) {
        int n = graph.length;
        ...
        try {
            while (newGroupsFormed && iter < maxIterations) {
                ...
                for (int i = 0; i < routes.size(); i++) indices.add(i);
    ...
    static Result finalizeRoute(double[][] cities, int[][] graph, Map<Long, Seed> globalSeeds) {
        ...
        // ----------------------------------------------------
        // SEED COMPATIBILITY GRAPH & GREEDY MWIS
        // ----------------------------------------------------
            ...
            if (!extended && path.size() < n) {
                ...
                for (int i = 0; i < n; i++) {
                    if (!visited[i] && graph[backNode][i] != -1 && graph[backNode][i] < minCost) {
                        // Check if connection crosses path
